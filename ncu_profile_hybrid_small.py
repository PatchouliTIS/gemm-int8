...
scale_a = torch.rand(M_blocks_hyb_s, K_blocks_256, dtype=torch.float32, device=device) * 0.1
scale_b = torch.rand(N_blocks_hyb_s, K_blocks_256, dtype=torch.float32, device=device) * 0.1
...
for _ in range(5):
    out = gemm_int8.blockwise_fused_matmul_hybrid_small(
